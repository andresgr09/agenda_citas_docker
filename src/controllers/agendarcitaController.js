    ...
    async (req, res) => {
        ...
        try {
            const [correoExistente, citaExistente, citaMismaFecha, citaSeleccionada] = await Promise.all([
                CitaAgendada.findOne({ where: { correo, documento: { [Op.ne]: numIdentificacion } } }),
                CitaAgendada.findOne({ where: { documento: numIdentificacion, tipo_documento: tipoDoc, estado_agenda: 'confirmada' }, order: [['fecha_solicitud', 'DESC']] }),
                CitaAgendada.findOne({ where: { fecha_cita, estado_agenda: 'confirmada' } }),
                CitaDisponible.findOne({ where: { id_cita_dispo: citaId } })
            ]);
            ...
            const mensaje = `
                ...
                <p><strong>Dirección de la sede:</strong> ${direccion}</p>
