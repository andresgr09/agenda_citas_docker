                    ...
                    <body>
                        <div id="modal" style="display: block; position: fixed; z-index: 1; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgb(0,0,0); background-color: rgba(0,0,0,0.4);">
                            <div style="background-color: #fefefe; margin: 15% auto; padding: 20px; border: 1px solid #888; width: 80%;">
                                <img src="/images/logo-migracion-colombia.png" alt="Logo" style="width: 250px; display: block; margin: 0 auto;">
...
//     const { radicado } = req.query;

//     try {
//         // Buscar la cita en CitasAgendadas usando el campo radicado
//         const cita = await CitaAgendada.findOne({ where: { radicado } });

//         if (!cita) {
//             console.error(`Cita no encontrada con radicado: ${radicado}`);
//             return res.status(404).json({ message: 'Cita no encontrada' });
//         }

//         // Obtener el id_cita_dispo_fk antes de eliminar la cita
//         const idCitaDispoFk = cita.id_cita_dispo_fk;
//         console.log(`id_cita_dispo_fk: ${idCitaDispoFk}`);

//         if (!idCitaDispoFk) {
//             console.error('id_cita_dispo_fk es undefined');
//             return res.status(500).json({ message: 'Error al cancelar la cita: id_cita_dispo_fk es undefined' });
//         }

//         // Eliminar la cita en CitasAgendadas
//         await cita.destroy();

//         // Cambiar el estado de la cita disponible a "disponible" utilizando id_cita_dispo_fk
//         const citaDisponible = await CitaDisponible.findOne({ where: { id_cita_dispo: idCitaDispoFk } });
//         if (citaDisponible) {
//             citaDisponible.estado_agenda = 'disponible';
//             await citaDisponible.save();
//             console.log(`Estado de la cita disponible actualizado a "disponible" para id_cita_dispo_fk: ${idCitaDispoFk}`);
//         } else {
//             console.error(`Cita disponible no encontrada con id_cita_dispo_fk: ${idCitaDispoFk}`);
//         }
